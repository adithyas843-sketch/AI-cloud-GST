...
def generate_insights(recon: pd.DataFrame) -> pd.DataFrame:
    columns=["Invoice Number", "GSTIN", "Exception", "Likely Cause", "Recommended Action", "Exposure Amount"]
    ...
    for _, r in recon[recon["Status"]=="Mismatch"].iterrows():
        ...
        rows.append({"Invoice Number":r["Invoice Number"],"GSTIN":r["GSTIN"],"Exception":issue,"Likely Cause":cause,"Recommended Action":action,"Exposure Amount":r["Exposure Amount"]})
